...
//		}
	   ...
	   List<Map<String, String>> list = DSPTcpUtils.getFaultWaveformDataList("10.128.5.200", 503, 66, waveformCodes);
	 ...
	 *            波形编号集合
	 */
	public static boolean  sendCmd(String cmdType, String cmdCode, String[] waveformCodes) {
	...
	public static List<Map<String, String>> getFaultWaveformDataList(String ip, int port, int length, String[] waveformCodes){
		boolean con=DSPTcpUtils.connect(ip, port);
		if(con==false){
			return null;
		}
		boolean send=DSPTcpUtils.sendCmd("A9", "00", waveformCodes);
		if(con==false){
			return null;
		}
