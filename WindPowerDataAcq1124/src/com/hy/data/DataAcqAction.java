...
public class DataAcqAction {
	...
	public static void updatedevicestate(int devid,String acqstate) {
			try {
				...
				Connection conn_trd = jdbc_trd.connection;
				Statement st_trd  = conn_trd.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
				String sql = "update   windpower_device  set last_acqtime=NOW() , acq_state='"+acqstate+"' where id="+devid;
				st_trd.execute(sql);
				st_trd.close();
				conn_trd.close();
			} catch (SQLException e) {
	...
	public static void getrw_role(int totcal_device_id) {
		...
		try {
			...
			Statement st_trd   = conn_trd.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
			String sql = "select  * from   windpower_device "
					+ " where   id="+totcal_device_id;
	...
	protected static boolean getmodbustcpstrbyip(int device_id, String ip, int port, int d_type, int startaddr,
		...
		try {
			...
			if (modbustcpstr == "" || modbustcpstr == null||modbustcpstr=="-1"||modbustcpstr.equals("connect")||modbustcpstr.equals("sendcodeerror")) {
			 ...
			 updatedevicestate(device_id,modbustcpstr);
			   ...
			   if((tcplenght==modbuslength&&d_type==2)||(tcplenght==(modbuslength-5)&&d_type!=2)){
					...
					//	System.out.println("发现故障=============================="+device_id+"="+ip+"=="+allidmap.get(device_id));
			     ...
			     //延迟2秒后插入 详情错误数据
				/*	 if(device_id==16){
			   	     LOGGER.info("事件记录标志=============="+device_id+"="+ip+"=="+errortimemap.get(device_id));
			   	     }*/
			     if(errortimemap.get(device_id)!=null&&errortimemap.get(device_id)==10){
			    	 //  System.out.println("开始插入事件记录=============="+device_id+"="+ip+"=="+allidmap.get(device_id));
					   LOGGER.info("开始插入事件记录=============="+device_id+"="+ip+"=="+allidmap.get(device_id)+"5秒后的故障数据"+ errordatastrmap.get(device_id));
			     ...
			     //延迟10秒读取DSP波录管理
			/*  if(device_id==16){
			    	    LOGGER.info("DSP录波管理=============="+device_id+"="+ip+"=="+dsperrortimemap.get(device_id)); 
			     }*/
			    ...
			    if(dsperrortimemap.get(device_id)!=null&&d_type!=2&&dsperrortimemap.get(device_id)==5){
			   	   //  System.out.println("开始插入DSP录波管理=============="+device_id+"="+ip+"=="+allidmap.get(device_id));
			   	     LOGGER.info("开始插入DSP录波管理=============="+device_id+"="+ip+"=="+allidmap.get(device_id)+"事件记录时刻的数据："+modbustcpstr);
			    	 updatedeviceerrorstate(device_id,"ERROR");
			    	 inserterDspErroecode(device_id,d_type,modbustcpstr);
			    	  errordatastrmap.put(device_id, modbustcpstr);
 			   	   ...
 			   	   //  System.out.println("结束插入DSP录波管理=============="+device_id+"="+ip+"=="+allidmap.get(device_id));
 			   	    LOGGER.info("结束插入DSP录波管理=============="+device_id+"="+ip+"=="+allidmap.get(device_id));
				...
				//双馈，全功率
					if (d_type != 2) {
						...
						//	LOGGER.info(device_id+"设备并网标志位前："+errordevicestrmap.get(device_id)+"值："+wc_jc_map.get(device_id));
							...
							//System.out.println(device_id+"="+ip+"中20_2位的值位" + bit20_2 + "   20_4值位" + bit20_4);
							...
							if (int38 != 0) {
								// bwzt=1;如果状态之前不是1 ，表示 开始状态开始记录时间
								if (gzztmap.get(device_id)==null||(gzztmap.get(device_id)!=null&&gzztmap.get(device_id) != 1)) {
									// 插入并网开始时间
			...
			//将实时库30分钟外的数据移到历史库中、复制数据到历史库，删除实时库
	...
	private static void updatedeviceerrorstate(int device_id, String string) {
		...
		try {
			JDBConnection jdbc_trd = new JDBConnection();
			Connection conn_trd = jdbc_trd.connection;
			Statement st_trd = conn_trd.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,
...
private static void starttimeupdate(int device_id, int d_type, String ip, int port) {
	...
	timestr=gc.get(GregorianCalendar.YEAR)+","+mm+","+gc.get(GregorianCalendar.DATE)+","+gc.get(GregorianCalendar.HOUR_OF_DAY)+","+gc.get(GregorianCalendar.MINUTE)+","+gc.get(GregorianCalendar.SECOND);
	timestr=timestr.substring(2, timestr.length());
	if (d_type == 2) {
		modbusstr = smi.WriteSunModbusTcpStrAll(ip, port, d_type,
				"31f4", timestr, "0");
	} else {
		// 672-677 /13179-13184
		modbusstr = smi.WriteSunModbusTcpStrAll(ip, port, d_type,
...
private static void inserttimecode(int device_id, String timestr,String databz) {
	...
	try {
		...
		Statement st_trd = conn_trd.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,
					ResultSet.CONCUR_UPDATABLE);
		String sql = "  insert into  windpower_duitime (device_id,timestr,dsstate)values("
			     +device_id+",'"
				+ timestr+"','"
				+ databz
				+ "')";
    ...
    private static int insertrunstatetime(int device_id,String string, String strtime) {
    	...
    	try {
    		...
    		Statement st_trd = conn_trd.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,
    					ResultSet.CONCUR_UPDATABLE);
    		ResultSet rs_trd=null;
    		String sql = "  insert into  windpower_runstatect (device_id,run_type,starttime)values("
    			     +device_id+",'"
    				+ string+"','"
    				+ strtime
    				+ "')";
...
private static boolean getdevicerunstate(int device_id){
	try {
    	...
    	Statement st_trd = conn_trd.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,
					ResultSet.CONCUR_UPDATABLE);
    	ResultSet rs_trd=null;
		String sql = "  select *  from windpower_formrundata where  createtime >=CURRENT_TIMESTAMP - INTERVAL 60 second and device_id="+device_id;
		rs_trd=st_trd.executeQuery(sql);
		boolean isct=true;
		while(rs_trd.next()){
			//return false;
...
private static long  getdeviceruntime(String string, int device_id) {
	...
	Date day = new Date();
	SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	SimpleDateFormat df01 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	  ...
	  long endtime=df.parse(endtimestrtmp).getTime();
		...
		Connection conn_trd = jdbc_trd.connection;
			Statement st_trd = conn_trd.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,
					ResultSet.CONCUR_UPDATABLE);
			ResultSet rs_trd=null;
		String sql = "select  *  from  windpower_runstatect  where device_id="+device_id  +"  and run_type='"+string
	    +"'  and  DATE_FORMAT( starttime, '%Y-%c-%d %H:%i:%s')>= DATE_FORMAT('"+ starttimestr +"' ,'%Y-%c-%d %H:%i:%s') and  (DATE_FORMAT( endtime, '%Y-%c-%d %H:%i:%s')>= DATE_FORMAT('"+endtimestr+"' ,'%Y-%c-%d %H:%i:%s') OR endtime is null) "
	    		+ " AND DATE_FORMAT( starttime, '%Y-%c-%d %H:%i:%s')<=DATE_FORMAT('"+endtimestr+"' ,'%Y-%c-%d %H:%i:%s')";
		rs_trd=st_trd.executeQuery(sql);
		while(rs_trd.next()){
		...
		//	String tmp_str_end=rs_trd.getString("endtime");
			...
			String sql01 = "select  *  from  windpower_runstatect  where device_id=" + device_id + " and run_type='"
					...
					+ "' ,'%Y-%c-%d %H:%i:%s')  and  (DATE_FORMAT( endtime, '%Y-%c-%d %H:%i:%s')>=DATE_FORMAT('" + endtimestr
					+ "' ,'%Y-%c-%d %H:%i:%s')   OR endtime is null)";
			...
			String sql02 = "select  *  from  windpower_runstatect  where device_id=" + device_id + " and run_type='"
					+ string + "' and  DATE_FORMAT( starttime, '%Y-%c-%d %H:%i:%s')<= DATE_FORMAT('" + starttimestr
					+ "' ,'%Y-%c-%d %H:%i:%s') and  DATE_FORMAT( endtime, '%Y-%c-%d %H:%i:%s')<=DATE_FORMAT('" + endtimestr
					+ "' ,'%Y-%c-%d %H:%i:%s') and DATE_FORMAT( endtime, '%Y-%c-%d %H:%i:%s')>=DATE_FORMAT('" + starttimestr
					+ "' ,'%Y-%c-%d %H:%i:%s') ";
	...
	private static void inserterDspErroecode(int device_id, int d_type, String modbustcpstr) {
	  ...
	  // LOGGER.info(device_id+"赋值："+errordevicestrmap.get(device_id));
		...
		if(errorcode.substring(0, 1).equals("1")&&wc_str.equals("1")){
				...
				List<Map<String, String>> list = DSPTcpUtils.getFaultWaveformDataList(dsp_wc_ip, Integer.parseInt(dsp_wc_port), 66, waveformCodes);
				if(list!=null){
					String excelurl = new CreateExcelByHashMap().createexcl(list,"网侧故障"+waveformCodes[1],devide_name);
	 	  ...
	 	  if(errorcode.substring(1,2).equals("1")&&jc_str.equals("1")){
				...
				List<Map<String, String>> list2 = DSPTcpUtils.getFaultWaveformDataList(dsp_jc_ip, Integer.parseInt(dsp_jc_port), 66, waveformCodes);
				if(list2!=null){
					String excelurl2 = new CreateExcelByHashMap().createexcl(list2,"机侧故障"+waveformCodes[1],devide_name);
			...
			if(errorcode.substring(1,2).equals("1")&&jc_str.equals("1")){
				...
				waveformCodes[1]=get4HexString(jc_261).split(",")[1];
			     LOGGER.info("机侧编号"+waveformCodes[1]);
				List<Map<String, String>> list = DSPTcpUtils.getFaultWaveformDataList(dsp_jc_ip, Integer.parseInt(dsp_wc_port), 66, waveformCodes);
				if(list!=null){
	...
	try {
		...
		Connection conn_trd = jdbc_trd.connection;
			Statement st_trd = conn_trd.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,
					ResultSet.CONCUR_UPDATABLE);
			ResultSet rs_trd=null;
		String sql = "select  t.ip,t.port,t1.name from  windpower_deviceinfo t ,windpower_device t1   where    t.device_id=t1.id and t.device_id="+device_id +"  and  t.d_type='dsp' order by t.dsp_type asc";
		rs_trd=st_trd.executeQuery(sql);
		 String name="";
		while(rs_trd.next()){
...
static String closeRundata() {
	...
	try {
		...
		Connection conn_trd = jdbc_trd.connection;
			...
			SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		    String endtimestr=df.format(day);
		String sql = "update  windpower_runstatect  set endtime='"+endtimestr+"' where  endtime is null";
		st_trd.execute(sql);
		st_trd.close();
		conn_trd.close();
	} catch (SQLException e) {
	...
	// TODO Auto-generated method stub
		...
		try {
			...
			String sqlinert="insert into windpower_errordataparse_dsp (device_id,device_name,error_type,error_excel,error_time)values("+device_id+",'"+device_name+"',"+type+",'"+excelurl+"','"+erroetimestr+"')";
	...
	private static String  geterrortype(int d_type,String modbustcpstr) {
		...
		try {
			...
			String sql = "select  et.id,et.errorname,ed.addr,ed.bitid,ap.name,ap.category from  windpower_errortype et,"
						+"windpower_errortypdetail  ed ,modbustcp_sk_app ap where et.id=ed.error_type   and ap.addr=ed.addr and ed.modbus_type=ap.modbus_type  and  ed.modbus_type="+d_type+"  and  (errorname like'%网侧%'  or errorname like'%机侧%')";
			rs_trd=st_trd.executeQuery(sql);
		    String  wc_error="0";
		    String jc_error="0"; 
			while(rs_trd.next()){
		...
		try {
			...
			for (int i = 0; i < errdatalist.size(); i++) {
				String sql = "insert into  windpower_errordataparse (" + "device_name," + "error_type,"
						...
						+ errdatalist.get(i).getError_excel() + "'," + errdatalist.get(i).getErrordata_id() + ",'"+ errdatalist.get(i).getCometime() +"')";
		...
		try {
			...
			while (rs_trd.next()) {
				...
				if (Integer.parseInt(modbusstr[addrnew]) > 0) {
					...
					if(li.size()>0){
					String bitstring = setbittrue(li,
							Integer.parseInt(data.split(",")[addrnew]));
					//bit位空，不存在特殊bit位
					// 地址值大于0
					// 所有位都是故障信息
					if (bitid == null || bitid == "") {
 ...
 */
	private static String geterrortime(String data, int modbus_type) {
		...
		try {
			...
			String sql = "select  *  from modbustcp_sk_app where  array_type="+array_type+" and  modbus_type=" + modbus_type;
			rs_trd = st_trd.executeQuery(sql);
			int i=0;
			while (rs_trd.next()) {
 ...
 */
		...
		try {
			...
			for (int i = 0; i < errdatarepairlist.size(); i++) {
				String sql = "insert into  windpower_errordatarepair (" + "device_name," + "error_type,"
						+ "error_column," + "error_name," + "error_excel," + "errordata_id," + "cometime"+")value" + "('"
						+ errdatarepairlist.get(i).getDevice_name() + "'," + errdatarepairlist.get(i).getError_type() + ",'"
						+ errdatarepairlist.get(i).getError_column() + "','" + errdatarepairlist.get(i).getError_name() + "','"
						+ errdatarepairlist.get(i).getError_excel() + "'," + errdatarepairlist.get(i).getErrordata_id() + ",'"+errdatarepairlist.get(i).getCometime()+"')";
				LOGGER.info("插入派单解析库：" + sql);
				st_trd.execute(sql);
				//派发邮件
				SimpleMailSender smail=new SimpleMailSender();
				smail.mailSend( errdatarepairlist.get(i).getDevice_name()+"="+errdatarepairlist.get(i).getError_name()+":"+errdatarepairlist.get(i).getError_column(), errdatarepairlist.get(i).getError_excel());
			}
			st_trd.close();
			conn_trd.close();
		} catch (Exception e) {
	...
	private static List<ModbusBit> getbittrue(int addr, int modbus_type) {
		List<ModbusBit> bitlist = new ArrayList();
		try {
			JDBConnection jdbc_trd = new JDBConnection();
			Connection conn_trd = jdbc_trd.connection;
			Statement st_trd = conn_trd.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
			ResultSet rs_trd = null;
			String sql = "select  * from modbustcp_sk_appparame where  modbus_type=" + modbus_type + " and  addr="
					+ addr;
			rs_trd = st_trd.executeQuery(sql);
			while (rs_trd.next()) {
