...
/*		   int code_h=codeleg/255;
	...
	public String[] dlhf(Socket socket) {
		...
		String[] loadcode = { "10", "18", "00", "06", "4D", "51", "49", "73", "64", "70", "03", "02", "00", "78", "00",
				"0A" };
		//playload长度
...
/*		int mqttlent=svhex.length+16;
	 ...
	 * @return
	 */
	public String[] sbdyhf(String [] dystr,Socket socket) {
		SunModbusTcpbyIp stp = new SunModbusTcpbyIp();
		...
		String[] strdy = stp.sendonload(hfcode, socket);
		if (strdy.length > 0) {
			String strfh = sztostr(strdy);
			System.out.println("设备订阅回复指令发送成功返回：" + strfh);
		} else {
			System.out.println("设备订阅回复指令发送失败！真正从新发送");
			for (int i = 0; i < 5; i++) {
				String[] str1 = stp.sendonload(hfcode, socket);
				if (str1.length > 0) {
					System.out.println("设备订阅成功！");
					;
				}
 ...
 */
	 ...
	 * @return
	 */
	private static String[] getcddatacode(CdhData chd) {
		// 设备名称
		int cdhid = chd.getCdid();
		int cdhlx = chd.getLx();
		int cdhlxlen = chd.getLxlen();
		int addrdata = chd.getValue();
		...
		if (addrstr != null && addrstr != "") {
			...
			// 对数据值处理
			String datatmp = Integer.toHexString(addrdata);
			String[] namestr = get4HexString(datatmp).split(",");
			if (cdhlxlen == 1) {
				String tmpstr = get4HexString(datatmp).split(",")[1];
				namestr = new String[1];
				namestr[0] = tmpstr;
			}
			nametmp = new String[cdhlxlen];
			System.arraycopy(namestr, 0, nametmp, cdhlxlen - namestr.length, namestr.length);
			// 长度不足，补0
			for (int i = 0; i < cdhlxlen - namestr.length; i++) {
				nametmp[i] = "00";
			}
		}
 ...
 * @return
 */
	private static String[] getcsdcode(int id, String[] csdcgbz,Device dev) {
		// 设备名称
		String tmpstr = "";
		String devip="";
		String[]   namehex=null;
	 ...
	 * @param dev
	 */
	public static void startthreadother(Device dev,Socket sck){
		//0表示上传运行数据，1表示上传历史数据,2表示设备基本信息及统计信息数据，无论有无故障都发送
	   // sendrundatacode(dev,2,sck,null,null);
		//0表示上传运行数据，1表示上传历史数据
	    sendrundatacode(dev,0,sck,null,null);
	...
	private static Device getDevicedata(int did) {
		...
		try {
			...
			ResultSet rs_trd = null;
			Statement st_trd = conn_trd.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
			String sql = "select 	t.*,t1.modbus_type from windpower_db.mqtt_device t  ,windpower_devicetype t1 ,windpower_device t2 where t2.device_type_id=t1.id and t.device_id=t2.id and  t2.run_state=0  and t.run_state=0 and t2.id="+did ;
			rs_trd = st_trd.executeQuery(sql);
			while (rs_trd.next()) {
 ...
 * @return
 */
private static List<CdhData> getBasedata(Device dev,Data_Acq datatmp){
	...
	List<CdhData> cdall = new ArrayList<CdhData>();
		if(data!=""&&data!=null){
			  ...
			  if(value38>0&&gzztmap.get(dev.getDevice_id())==1){
		...
		while(rs_trd.next()){
			CdhData cdhone=new CdhData();
			  ...
			  if(cdhone.getModbus_type()==2){
				  ...
				  cdhone.setValue(Integer.parseInt(datastr[Integer.parseInt(cdhone.getAddr())-12788]));
				 }
			  }else{
				  if(cdhone.getAddr().split("/").length>1){
				     ...
				     cdhone.setValue(Integer.parseInt(datastr[Integer.parseInt(cdhone.getAddr())]));
				  }
			  }
			  cdall.add(cdhone);
		}
		...
		try {
			...
			while (rs_trd.next()) {
				...
				if (addrone.length > 1) {
					...
					valuetmp = Integer.parseInt(datastr[Integer.parseInt(addrtmp)]);
					cdhone.setValue(valuetmp);
				}
				// 出故障大于0的地址才需要发送
				if (valuetmp > 0) {
 ...
 */
	public static String[] getZT(String zt) {
		...
		try {
			...
			if(zt.equals("DY")){
				//订阅主题设置为01
				String[] basecode={"82","24","00","01","00","11"};
