	...
	$.post(menu_url,function(data){
		...
		if(lid == "9"){
			if(qx == 1){
				...
				$("#iframe").attr("style","padding: 120px;width:100%;height:22200px;");
			}else{
...
function clsdiv(){
	...
	$("#alertwindow").hide();
	debugger;
	var deviceid=window.document.getElementById("deviceIds").value;
	if(deviceid!=""){
...
<body>
	 <div id="loading"  class="loadingTop" style="top:50%;left:50%;position:relative; display:none;"></div>
 ...
 <!-- 头部内容home -->
    ...
    <iframe id="iframe" scrolling="no" frameborder="0"  src="" ></iframe>
   	...
   	<div id="grayT"></div>
    <!-- 弹出框 -->
    <div class="popup poput_box_b Min_w tablee" id="alertwindow" style="display:none;">
		<p class='poput_title'><span id="titletxt"></span><a style='cursor:pointer;' onclick='clsdiv()'></a></p>
		<div class="poput_box_height" id="contentdiv">
					
		</div>
	</div>
    <!-- 弹出框 end-->
    <!-- 弹出框 专家库 -->
    <div class="popup poput_box_b Min_w tablee" id="alertwindow2" style="display:none;">
		<p class='poput_title'><span id="titletxt2"></span><a style='cursor:pointer;' onclick='clsdiv2()'></a></p>
		<div class="poput_box_height" id="contentdiv2">
