...
function loadPage(id){
	if(id==null){
	 ...
	 var url = "<%=basePath%>PageSet/listModbusSkApp.do?modbus_type="+id;
	/* $.get(url,function(data){
		var result = JSON.parse(data);
		var varlist = result.responseData.msList;
		var checkedStr = '';
		...
		for(var i = 0;i < varlist.length;i++){
			str +="<tbody>"
			+"<tr>"
			+"<td style='width:50px;'>"+(i+1)+"</td><td>"+varlist[i].name+"</td><td> <label class='label_input padding_style'><input type='checkbox' class='input_chk' name='ids' value='"+varlist[i].id+"'"
			if(varlist[i].isbasicparm == "1"){
				str += "checked = 'checked'";
				if(checkedStr=='') checkedStr += varlist[i].id;
			  	else checkedStr += ',' + varlist[i].id;
			}
			str +="/><span class='style01'></span></label></td>"
			+"</tr>"
			+"</tbody>";
		}
	...
	$.ajax({
		type:'post',
		url:'<%=basePath%>PageSet/listModbusSkApp.do',
		data:{"modbus_type":id},
		success:function(data){
			$(".table_box").show();
			$("#loading").hide();
...
function save(){
	...
	var numberYiBai=str.split(",");
	if(numberYiBai.length>100){
		alert("基本参数不能超过100个，您设置的基本参数太多，请重新设置！");
		window.location.reload();
		return false;
	}
	var oldids = $("#oldids").val();
	var url = "<%=basePath%>PageSet/updateModbusSkApp.do?paramStr="+str+"&oldparm="+oldids;
    ...
    <div class="main_box clearfix">
     <!-- 左侧菜单 -->
        ...
        <div class="right_main_div0">
            <div class="W_width">
                <!-- 头部切换标签 -->
                <p class="P_Label clearfix input_style">基本参数配置</p>
                <div class="main_title_box" id="main_title_box">
                    <!-- <ul>
                        <li class="li_style"><a onclick="loadPage()">双馈</a></li>
