...
function autoHeight(){
    ...
    $("#oscDsp1-2").attr("style","height:"+(height/2 - 40)+"px");
    $("#oscDsp2-2").attr("style","height:"+(height/2 - 40)+"px");  
...
<!-- 头部内容home -->
	<div class="hd_box">
		<div class="logo_box">
			<a href="#"><img src="<%=basePath%>/images/logo.png" style="margin-top: -18%;"></a>
		</div>
		<div class="nav_box">
			<ul>
	...
	<div class="main_box clearfix">
		...
		<div class="W_width">
			...
			<p style="width: 80%; float: left;" class="P_Label p_spanw clearfix">
				...
				<!-- select 下拉菜单 -->
				<strong style="margin-top: 4px; float: left; width: 10% !important" class="stronta">
					<select id="samplingFrequency" name="samplingFrequency" style="width: 140px;">
			...
			<p style="width: 19%; float: right;" class="P_Label clearfix">
				<span style="border: none;">波形名称</span>
			...
			<div class="table_box" id="autoarea">
				<!-- 数据趋势图 -->
				<div class="box_lef011 bj_box" style="width: 80%; background-color: black;">
					<div class="w_w">
				...
				<div class="box_right box_right011" style="width: 18%">
					...
					<div id="oscDsp1-1" class="T_table table_boxb" style="height: 40px;">
						<table id="table ">
							<tbody>
								<tr>
									<td style="width: 21%;padding-left: 5%;display:none;">显示</td>
									<td style="width: 21%;padding-left: 5%;">选择</td>
									<td>网侧波形</td>
								</tr>
							</tbody>
						</table>
						...
						<table id="table ">
							<tbody>
								<c:forEach items="${webList}" var="wl" varStatus="ind">
									<tr>
										<td style="width: 21%;padding-left: 6%; display:none;">
										...
										<td style="width: 21%;padding-left: 6%;"><label class="label_input label_style11"> <input
										...
										<td style="padding-left: 6%;" title="${wl.name}">
											<c:if test="${wl.name.length() > 8}">
												${fn:substring(wl.name, 0, 7)}...
											</c:if>
											<c:if test="${wl.name.length() <= 8}">
												${wl.name}
											</c:if>
					...
					<!-- 示波器机测 -->
					<div id="oscDsp2-1" class="B_table table_boxb" style="height: 40px;">
						<table id="tableA">
							<tbody>
								<tr>
									<td style="width: 21%;padding-left: 5%; display:none;">显示</td>
							...
							<tbody>
								<c:forEach items="${machineList}" var="ml" varStatus="ind">
									<tr>
										...
										<td style="padding-left: 6%;" title="${ml.name}">
											<c:if test="${ml.name.length() > 8}">
												${fn:substring(ml.name, 0, 7)}...
											</c:if>
											<c:if test="${ml.name.length() <= 8}">
												${ml.name}
											</c:if>
						...
						<table>
							<tbody>
								...
								<c:forEach items="${webDefaultList}" var="wl" varStatus="ind">
									...
									<tr>
										<td width="58px;"><label class="label_input label_style11" style="position:relative;"> <input
												class="input_chk" type="checkbox" name="dspDefault1IsShow" onclick="showHideLine('dsp1DefaultWaveformCodes', '${wl.code}', this);"/> <span></span>
										</label></td>
										<td title="${wl.name}">
											<c:if test="${wl.name.length() > 12}">
												${fn:substring(wl.name, 0, 11)}...
											</c:if>
											<c:if test="${wl.name.length() <= 12}">
												${wl.name}
											</c:if>
						...
						<table>
							<tbody>
								...
								<c:forEach items="${machineDefaultList}" var="ml" varStatus="ind">
									...
									<tr>
										<td width="58px;"><label class="label_input label_style11"><input
												class="input_chk" type="checkbox" name="dspDefault2IsShow" onclick="showHideLine('dsp2DefaultWaveformCodes', '${ml.code}', this);"/> <span></span>
										</label></td>
										<td title="${ml.name}">
											<c:if test="${ml.name.length() > 12}">
												${fn:substring(ml.name, 0, 11)}...
											</c:if>
											<c:if test="${ml.name.length() <= 12}">
												${ml.name}
											</c:if>
...
<%-- 	<script type="text/javascript"  src="<%=basePath%>/jsp/windFieldPreview/oscilloscope/canvasjs.min.js"></script> --%>
	...
	<div id="messages"></div>
	<div id="maxXDataNum" style="position: relative; z-index: 1; color: gray; height: 35px; width: 50px; font-size: 24px; font-weight: 400; display: none">
		1000
	</div>
</body>
</html>