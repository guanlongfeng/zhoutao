...
public class ReportStatServiceImpl implements ReportStatService {
	...
	public String getDateAreaSql(String timename){
		...
		if("日".equals(timename)){//天
			...
			date_area_sql = "YEARWEEK(date_format(createtime,'%Y-%m-%d')) = YEARWEEK(now()) and createtime NOT LIKE CONCAT('%"+mintimestr+"%')";
			...
			try {
		        ...
		        if(nowDate.before(midDate)){
		        	mintimestr=firstday+minstr;
		        	date_area_sql= "TO_DAYS(createtime) BETWEEN TO_DAYS('"+firstday+"') and TO_DAYS('"+midday+"') and createtime NOT LIKE CONCAT('%"+mintimestr+"%')";
		        }else if(nowDate.after(midDate)){
		        	mintimestr=midday+minstr;
		        	date_area_sql= "TO_DAYS(createtime) BETWEEN TO_DAYS('"+midday+"') and TO_DAYS('"+lastday+"') and createtime NOT LIKE CONCAT('%"+mintimestr+"%')";
		        }
			} catch (Exception e) {
				e.printStackTrace();
			}
	        ...
	        String firstday = format.format(cal_1.getTime());
	        mintimestr=firstday+minstr;
			date_area_sql = "DATE_FORMAT(createtime,'%Y%m') = DATE_FORMAT(CURDATE( ) ,'%Y%m') and createtime NOT LIKE CONCAT('%"+mintimestr+"%')";
			...
			try {
				...
				if(nowyearday.before(midyearday)){
					...
					date_area_sql= "TO_DAYS(createtime) BETWEEN TO_DAYS('"+firstdayStr+"') and TO_DAYS('"+midyeardayStr+"') and createtime NOT LIKE CONCAT('%"+mintimestr+"%')";
				}else if(nowyearday.after(midyearday)){
					mintimestr=midyeardayStr+minstr;
					date_area_sql= "TO_DAYS(createtime) BETWEEN TO_DAYS('"+midyeardayStr+"') and TO_DAYS('"+enddayStr+"') and createtime NOT LIKE CONCAT('%"+mintimestr+"%')";
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public List<FormRunData> listFormRunData(String timename) {
 		...
 		return updaterundata(reportStatDao.listFormRunData(formRunData),time_sql);
